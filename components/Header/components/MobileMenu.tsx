...
export default function MobileMenu() {
  ...
  return (
    <div className="absolute top-full left-0 w-full bg-primary flex flex-col items-center gap-4 py-6 lg:hidden z-50">
      ...
      <Link href="/" onClick={() => dispatch(setMobileMenu(false))}>{t("about")}</Link>
      <ServiceDropdownButton/>
      <Link href="/" onClick={() => dispatch(setMobileMenu(false))}>{t("team")}</Link>
      <Link href="/" onClick={() => dispatch(setMobileMenu(false))}>{t("contact")}</Link>
